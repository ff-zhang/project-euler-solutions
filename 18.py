...
def find_greater_sum(n1, n2, base):
    if n1 > n2:
        return n1 + base
...
04 62 98 27 23 09 70 98 73 93 38 53 60 04 23
"""
triangle = list(filter(None, [list(map(int, row.split())) for row in triangle.split("\n")]))
triangle.reverse()

for i, row in enumerate(triangle):
    for j in range(len(row) - 1):
        triangle[i+1][j] = find_greater_sum(row[j], row[j+1], triangle[i+1][j])
