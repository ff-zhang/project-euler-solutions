...
def calculate_product(matrix, row, column, direction):
    grid = matrix
    product = 1
    consecutive = 4
    try:
...
directions = ["down", "right", "diagonal_up", "diagonal_down"]
for i in range(len(directions)):
    direction = directions[i]

    for i in range(20):
        for j in range(20):
            temp = calculate_product(grid, i, j, direction)
            if temp != None and temp > max_product:
                max_product = temp
